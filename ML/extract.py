...
# Load CSV
df = pd.read_csv('./PhiUSIIL_Phishing_URL_Dataset.csv')[['URL', 'label']]
...
for _, row in df.iterrows():
    ...
    if features:
        rows.append(features + [row['label']])
    else:
        failed += 1

print(f"Thành công: {len(rows)}, Thất bại: {failed}")

new_df = pd.DataFrame(rows, columns=FEATURE_NAMES + ['label'])
new_df.to_csv('dataset_custom.csv', index=False)
print("Đã lưu dataset_custom.csv")
